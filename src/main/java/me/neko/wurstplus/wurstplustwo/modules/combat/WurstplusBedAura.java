...
import me.neko.wurstplus.wurstplustwo.util.BlockUtil;
...
public class WurstplusBedAura extends Module {
    ...
    public void break_bed() {
        BlockPos pos;
        for(Iterator var1 = ((List) BlockInteractHelper.getSphere(get_pos_floor(mc.player), (float)this.range.get_value(1), this.range.get_value(1), false, true, 0).stream().filter(WurstplusBedAura::is_bed).collect(Collectors.toList())).iterator(); var1.hasNext(); BlockUtil.openBlock(pos)) {
