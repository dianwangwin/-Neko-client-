...
public class ModuleArrayList extends Pinnable { //This class name can't be called "ArrayList"
	...
	public void render() {
		...
		List<Module> pretty_modules = NekoHax.get_hack_manager().get_array_active_hacks().stream()
			.sorted(Comparator.comparing(modules -> get(modules.array_detail() == null ? modules.get_tag() : modules.get_tag() + NekoHax.g + " [" + NekoHax.r + modules.array_detail() + NekoHax.g + "]" + NekoHax.r, "width")))
			.collect(Collectors.toList());

		int count = 0;

		if (NekoHax.get_setting_manager().get_setting_with_tag("HUD", "HUDArrayList").in("Top R") || NekoHax.get_setting_manager().get_setting_with_tag("HUD", "HUDArrayList").in("Top L") ) {
		...
		for (Module modules : pretty_modules) {
			...
			if (modules.get_category().get_tag().equals("Client")) {
				continue;
			}
			...
			if (flag) {
				String module_name = (
					...
					modules.get_tag() + NekoHax.g + " [" + NekoHax.r + modules.array_detail() + NekoHax.g + "]" + NekoHax.r
				);

				if (NekoHax.get_setting_manager().get_setting_with_tag("HUD", "HUDArrayList").in("Free")) {
					...
					if (NekoHax.get_setting_manager().get_setting_with_tag("HUD", "HUDArrayList").in("Top R")) {
						mc.fontRenderer.drawStringWithShadow(module_name, scaled_width - 2 - mc.fontRenderer.getStringWidth(module_name), 3 + count * 10, new Draw.ClientColor(nl_r,nl_g,nl_b,nl_a).hex());
						count++;
					}
					if (NekoHax.get_setting_manager().get_setting_with_tag("HUD", "HUDArrayList").in("Top L")) {
						mc.fontRenderer.drawStringWithShadow(module_name, 2, 3 + count * 10, new Draw.ClientColor(nl_r,nl_g,nl_b,nl_a).hex());
						count++;
					}
					if (NekoHax.get_setting_manager().get_setting_with_tag("HUD", "HUDArrayList").in("Bottom R")) {
						mc.fontRenderer.drawStringWithShadow(module_name, scaled_width - 2 - mc.fontRenderer.getStringWidth(module_name), scaled_height - (count * 10), new Draw.ClientColor(nl_r,nl_g,nl_b,nl_a).hex());
						count++;
					}
					if (NekoHax.get_setting_manager().get_setting_with_tag("HUD", "HUDArrayList").in("Bottom L")) {
						mc.fontRenderer.drawStringWithShadow(module_name, 2, scaled_height - (count * 10), new Draw.ClientColor(nl_r,nl_g,nl_b,nl_a).hex());
						count++;
					}
				}
